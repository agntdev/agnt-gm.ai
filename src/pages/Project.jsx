...
export default function Project() {
  ...
  useEffect(() => {
    setProject(fixture);
    let cancelled = false;
    api.getProject(slug).then((live) => {
      if (cancelled || !live) return;
      setProject((prev) => ({ ...prev, ...live, sym: live.token_symbol || prev.sym, name: live.name || prev.name }));
    });
    api.listProjectTasks(slug).then((live) => {
      if (cancelled || !live?.tasks?.length) return;
      // keep our fixture tasks; live shape differs. No-op for now.
    });
    api.projectLeaderboard(slug).then(() => {});
              ...
              <div>
                <div className="about-facts">
                  ...
                  <div className="fact-row">
                    ...
                    <span className="v">
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                        <span style={{ width: 18, height: 18, borderRadius: 4, background: project.tone?.bg, display: "grid", placeItems: "center", fontSize: 9, fontWeight: 800, color: project.tone?.fg }}>
                          {project.creatorAlias?.slice(0, 2).toUpperCase()}
                        </span>
                        {project.creatorAlias}
                      </span>
                    </span>
                  </div>
                  {project.creatorBio && (
                  ...
                  <div className="fact-row">
                    ...
                    <span className="v" style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11 }}>
                      <Icon name="git_branch" size={11} /> {project.repo}
                    </span>
                  </div>
                  {project.homepage && (
                    <div className="fact-row">
                      <span className="l">Homepage</span>
                      <span className="v" style={{ fontFamily: "JetBrains Mono, monospace", fontSize: 11 }}>
                        ↗ {project.homepage}
                      </span>
                    </div>
                  ...
                  {project.deployedTo && (
                    <div className="fact-row">
                      <span className="l">Deployed to</span>
                      <span className="v" style={{ fontSize: 11.5 }}>{project.deployedTo}</span>
                    </div>
                  )}
                  {project.docsUrl && (
                    <div className="fact-row">
                  ...
                  <div className="fact-row">
                    ...
                    <span className="v">{project.license}</span>
                  </div>
                </div>

                {project.stack && (
                  <div className="about-facts" style={{ marginTop: 12 }}>
                    <div className="about-fact-head">Tech stack</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: 14 }}>
                      {project.stack.map((s) => (
                        <span key={s} className="tag-chip" style={{ background: "var(--bg)", border: "1px solid var(--border)" }}>{s}</span>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div className="about-facts" style={{ marginTop: 12 }}>
                  ...
                  <div className="fact-row">
                    ...
                    <span className="v">${project.sym}</span>
                  </div>
                  <div className="fact-row">
                    <span className="l">Price</span>
                    <span className="v">
                      ${project.price.toFixed(6)}
                      <span className={positive ? "pos" : "neg"} style={{ fontSize: 10.5, marginLeft: 6 }}>
                        {positive ? "+" : ""}{project.change.toFixed(1)}%
                      </span>
                    </span>
                  </div>
                  <div className="fact-row">
                    <span className="l">Mcap</span>
                    <span className="v">{project.mcap}</span>
                  </div>
                  <div className="fact-row">
                    <span className="l">Holders</span>
                    <span className="v">{project.holders}</span>
                  </div>
                  <div className="fact-row" style={{ borderBottom: "none" }}>
                    ...
                    <span className="v" style={{ color: "var(--accent-fg)", fontWeight: 800 }}>
                      ...
                      <span style={{ color: "var(--fg-muted)", fontWeight: 600, marginLeft: 6 }}>+ {project.rewardPool?.tokens}</span>
                    </span>
                  </div>
                </div>
              </div>
